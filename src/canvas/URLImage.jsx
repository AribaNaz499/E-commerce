...
const URLImage = ({ el, isSelected, onSelect, onChange }) => {
  ...
  useEffect(() => {
    if (img && (!el.crop || el.crop.width <= 1)) {
      onChange({
        ...el,
  ...
  const handleTransformEnd = () => {
    ...
    if (isCropping) {
      ...
      onChange({ ...el, crop: newCrop, x: node.x(), y: node.y() });
    } else {
      onChange({
        ...el,
        x: node.x(),
  ...
  return (
    <Group>
      {isCropping && (
        <Image image={img} x={el.x} y={el.y} width={el.width} height={el.height} opacity={0.3} listening={false} />
      )}
      <Image
        id={el.id}
        ref={shapeRef}
      ...
      {isCropping && (
        <Group onClick={() => setIsCropping(false)} x={el.x} y={el.y + el.height + 15}>
          <Rect width={100} height={32} fill="#fbbf24" cornerRadius={6} />
          <Text width={100} height={32} text="APPLY CROP" fill="black" align="center" verticalAlign="middle" fontStyle="bold" fontSize={12} />
        </Group>
      )}
    </Group>
  );
