...
import { ChevronRight, Trash2, Minus, Plus, Download } from 'lucide-react';
...
const stripePromise = loadStripe('pk_test_51Qw9P6GvU2n7E7X6m0V2n8Q...'); 
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div className="flex flex-col gap-2">
                                    ...
                                    <div className="flex">
                                        <select name="countryCode" value={formData.countryCode} onChange={handleInputChange} className="px-3 rounded-l-xl border border-r-0 border-slate-200 bg-slate-50 text-slate-600 outline-none">
                                ...
                                <div className="flex items-center justify-between p-5 border-2 border-rose-600 bg-rose-50/30 rounded-2xl">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-xs text-slate-500">Fixed Charges</p>
                                        </div>
                                    </div>
                                    <span className="font-black text-lg text-slate-900">Rs. {finalShipping}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex-1 w-full lg:sticky lg:top-10">
                        <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100">
                            ...
                            <div className="space-y-6 max-h-[500px] overflow-y-auto pr-2 mb-6">
                                {cartItems.map((item) => (
                                    <div key={item.id} className="p-4 border border-slate-100 rounded-2xl">
                                        <div className="flex gap-4">
                                            ...
                                            <div className="flex-1 min-w-0">
                                                ...
                                                <div className="flex justify-between items-center mt-2">
                                                    <p className="font-bold text-rose-600 text-sm">Rs. {item.price}</p>
                                                    <div className="flex items-center border rounded-lg bg-slate-50 scale-90">
                            ...
                            <div className="pt-6 border-t border-slate-100 space-y-3">
                                ...
                                <div className="flex justify-between text-sm text-slate-600">
                                    <span>Shipping</span>
                                    <span className="font-semibold text-green-600">Rs. {finalShipping.toLocaleString()}</span>
                                </div>
                                <div className="flex justify-between items-center pt-4 border-t border-slate-50">
                                    <span className="font-bold text-slate-900">Total Amount</span>
                                    <span className="font-black text-2xl text-rose-600">Rs. {total.toLocaleString()}</span>
                                </div>
                                <button 
                                    onClick={handleContinueToPayment} 
                                    className="w-full bg-rose-950 text-white py-4 rounded-2xl font-bold mt-4 hover:bg-rose-900 shadow-lg transition-all active:scale-[0.98]"
                                >
                                    Continue to Payment
                                </button>
                            </div>
